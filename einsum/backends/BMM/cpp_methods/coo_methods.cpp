...
void coo_matmul(double* A_data, int A_rows, int A_cols,
    ...
    for (int i = 0; i < A_rows; ++i) {
        ...
        for (int j = 0; j < B_rows; ++j) {
            int row_B = static_cast<int>(B_data[j * B_cols]);
            int col_B = static_cast<int>(B_data[j * B_cols + 1]);
            double val_B = B_data[j * B_cols + 2];

            if (col_A == row_B) {
                auto key = std::make_pair(row_A, col_B);
                C_dict[key] += val_A * val_B;
            }
        }
    }

    // Prepare output C_data as a 2D array
    std::vector<std::tuple<int, int, double>> result_data;
    for (const auto& [key, value] : C_dict) {
        result_data.push_back(std::make_tuple(key.first, key.second, value));
    }
