...
@boundscheck(False)
@wraparound(False)
def c_single_einsum_legacy(double[:] data, int rows, int cols, int[:] shape, bytes notation):
    cdef const double* data_ptr = &data[0]
    cdef const int* shape_ptr = &shape[0]
    cdef double *result_data
    cdef int result_rows, result_cols
    ...
    single_einsum_legacy(data_ptr, rows, cols, notation, shape_ptr, &result_data, &result_rows, &result_cols, &new_shape, &new_shape_size)
...
@boundscheck(False)
@wraparound(False)
def c_coo_bmm_legacy(double[:] A_data, int A_rows, int A_cols,
              double[:] B_data, int B_rows, int B_cols) -> np.ndarray:
    cdef const double* A_data_ptr = &A_data[0]
    cdef const double* B_data_ptr = &B_data[0]
    cdef double* C_data_ptr
    ...
    coo_bmm_legacy(A_data_ptr, A_rows, A_cols, B_data_ptr, B_rows, B_cols, &C_data_ptr, &C_rows, &C_cols)
