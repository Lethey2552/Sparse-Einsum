...
def c_single_einsum(
    ...
    if legacy:
        legacy_single_einsum(data_ptr, rows, cols, 
                             notation, shape_ptr, 
                             &result_data, &result_rows, &result_cols, 
                             &new_shape, &new_shape_size)
    else:
        single_einsum(data_ptr, rows, cols, 
                      notation, shape_ptr, 
                      &result_data, &result_rows, &result_cols, 
                      &new_shape, &new_shape_size)
...
def c_coo_bmm(
        ...
        bint legacy) -> np.ndarray:

    cdef const double* A_data_ptr = &A_data[0]
    cdef const double* B_data_ptr = &B_data[0]
    cdef double* C_data_ptr
    cdef int C_rows, C_cols
    cdef bool run_legacy = legacy

    coo_bmm(A_data_ptr, A_rows, A_cols, 
            B_data_ptr, B_rows, B_cols, 
            &C_data_ptr, &C_rows, &C_cols,
            run_legacy)
...
def c_reshape(
    ...
    if legacy:
        legacy_reshape(data_ptr, data_rows, data_cols, 
                       shape_ptr, shape_length, 
                       new_shape_ptr, new_shape_length, 
                       &result_data, &result_rows, &result_cols)
    else:
        reshape(data_ptr, data_rows, data_cols, 
                shape_ptr, shape_length, 
                new_shape_ptr, new_shape_length, 
                &result_data, &result_rows, &result_cols)
